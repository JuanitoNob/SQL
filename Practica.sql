...
create table kcv.grupos(
	id serial primary key,
	grupo VARCHAR (50) 
);

create table kcv.marca(
	id serial primary key,
	marca VARCHAR (50),
	id_grupo INT
);

create table kcv.modelo(
	id serial primary key,
	modelo VARCHAR (50),
	id_marca INT
);

create table kcv.color(
	id serial primary key,
	color VARCHAR (50)
);

create table kcv.aseguradora(
	id serial primary key,
	aseguradora VARCHAR (50)
);

create table kcv.moneda(
	id serial primary key,
	moneda VARCHAR (50)
);


create table kcv.revisiones(
	id serial primary key,
	matricula VARCHAR (10),
	kms_revision INT,
	fecha_revision DATE,
	importe_revision INT ,
	id_moneda INT
);

create table kcv.polizas(
	id serial primary key,
	matricula VARCHAR (10),
	id_aseguradora INT,
	n_poliza INT,
...
-- CREAMOS LAS CONEXIONES ENTRE LAS TABLAS

alter table kcv.vehiculos add constraint id_color_color foreign key (id_color) references kcv.color(id);
alter table kcv.marca add constraint id_grupo_grupos foreign key (id_grupo) references kcv.grupos(id);
alter table kcv.modelo add constraint id_marca_marca foreign key (id_marca) references kcv.marca(id);
alter table kcv.vehiculos add constraint id_modelo_modelo foreign key (id_modelo) references kcv.modelo(id);
alter table kcv.polizas add constraint id_aseguradora_aseguradora foreign key (id_aseguradora) references kcv.aseguradora (id);
alter table kcv.revisiones add constraint id_moneda_moneda foreign key (id_moneda) references kcv.moneda (id);
alter table kcv.polizas add constraint pmatricula_vmatricula foreign key (matricula) references kcv.vehiculos(matricula);
alter table kcv.revisiones add constraint rmatricula_vmatricula foreign key (matricula) references kcv.vehiculos(matricula);


-- CREAMOS LA TABLA COCHE E INSERTAMOS LOS DATOS

CREATE TABLE kcv.coches(
...
INSERT INTO kcv.coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
-- INSERTAMOS LOS DATOS EN LAS TABLAS NORMALIZADAS	

insert into kcv.moneda (moneda) select moneda from kcv.coches c group by moneda;

insert into kcv.grupos (grupo) select grupo from kcv.coches c group by grupo;

insert into kcv.color (color) select color from kcv.coches group by color;

insert into kcv.aseguradora (aseguradora) select aseguradora from kcv.coches group by aseguradora;


insert into kcv.marca (marca, id_grupo)
select 
	c.marca,
	g.id
from kcv.coches c
inner join kcv.grupos g on c.grupo = g.grupo
group by c.marca, g.id
order by g.id, marca;


insert into kcv.modelo (modelo, id_marca)
select 
	c.modelo,
	m.id
from kcv.coches c
inner join kcv.marca m on c.marca = m.marca
group by c.modelo, m.id
order by m.id, c.modelo;


insert into kcv.vehiculos (matricula, id_modelo, fecha_compra, id_color, kms_totales)
select 
	c.matricula,
	m.id,
	c.fecha_compra,
	...
	c.kms_totales
from kcv.coches c
inner join kcv.modelo m on c.modelo = m.modelo
inner join kcv.color col on c.color = col.color
group by c.matricula, m.id, col.id, c.fecha_compra, c.kms_totales
order by c.matricula, m.id, col.id;


insert into kcv.polizas (matricula, id_aseguradora, n_poliza, fecha_alta_seguro)
select 
	v.matricula,
	a.id,
	c.n_poliza,
	c.fecha_alta_seguro
from kcv.coches c
inner join kcv.vehiculos v on c.matricula = v.matricula 
inner join kcv.aseguradora a on c.aseguradora = a.aseguradora 
group by v.matricula, a.id, c.n_poliza, c.fecha_alta_seguro 
order by v.matricula, a.id;


insert into kcv.revisiones (matricula, kms_revision, fecha_revision, importe_revision, id_moneda)
select 
	v.matricula,
	c.kms_revision,
	c.fecha_revision,
...
from kcv.coches c
inner join kcv.vehiculos v on c.matricula = v.matricula 
inner join kcv.moneda m on c.moneda = m.moneda
group by v.matricula, c.kms_revision, c.fecha_revision, c.importe_revision, m.id 
order by v.matricula, m.id;
...
select 
	...
	p.n_poliza, 
	a.aseguradora,
	max(p.fecha_alta_seguro) as ultima_fecha_alta_seguro 
from kcv.vehiculos v
inner join kcv.color c on v.id_color = c.id
inner join kcv.modelo m on v.id_modelo = m.id
inner join kcv.marca mar on m.id_marca = mar.id 
inner join kcv.grupos g on mar.id_grupo = g.id
inner join kcv.polizas p on v.matricula = p.matricula
inner join kcv.aseguradora a on p.id_aseguradora = a.id
group by v.matricula, fecha_compra,kms_totales, color, modelo, marca, grupo, n_poliza, aseguradora;
